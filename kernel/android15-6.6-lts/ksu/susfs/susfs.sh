...
# ======================================================
# 🧬 SuSFS — shared apply logic (any KSU fork, android15-6.6-lts)
# ======================================================
# Repo: https://gitlab.com/simonpunk/susfs4ksu
# Ported from the android14-6.1-lts variant of this script.
#
# Only ReSukiSU is supported on 6.6 for now: pershoot's KernelSU-Next
...
if [ "$KERNEL_VARIANT" = "SUKISU" ]; then
    error "SuSFS+SukiSU is not supported on kernel 6.6 yet (no sukisu.sh integration) — build without SuSFS or use ReSukiSU."
elif [ "$KERNEL_VARIANT" = "KSUNEXT" ]; then
    error "SuSFS+KernelSU-Next is not supported on kernel 6.6 (pershoot's susfs fork has no 6.6 branch) — build without SuSFS or use ReSukiSU."
fi
...
PATCHER_DIR="${LUMINAIRE_PATCH_DIR}/kernel/android15-6.6-lts/ksu/susfs"
...
if [ ! -f "$KERNEL_PATCH" ]; then
    ...
    # Unlike 5.15/6.1, the 6.6 SuSFS patch's context already includes the
    # ACK vendor-hook includes (trace/hooks/blk.h, trace/hooks/fs.h) that
    # this tree carries in fs/namespace.c — no pre/post-patch dance needed.
